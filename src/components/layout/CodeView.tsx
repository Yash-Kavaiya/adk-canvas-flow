...
export const CodeView: React.FC = () => {
  ...
  const handleExportBundle = async () => {
    const zip = new JSZip();
    const projectName = `${currentProject?.name || 'adk_agents'}`;
    zip.file('main.py', generatedCode || '# Generated by ADK Builder\n');
    const req = `google-adk>=1.0.0\npython-dotenv>=1.0.0\nrequests>=2.31.0`;
    zip.file('requirements.txt', req);
    const docker = `FROM python:3.11-slim\n\nWORKDIR /app\n\nCOPY requirements.txt .\nRUN pip install -r requirements.txt\n\nCOPY main.py .\n\nCMD ["python", "main.py"]`;
    zip.file('Dockerfile', docker);
    const readme = `# ${projectName}\n\nGenerated with ADK Builder.\n\n## Run locally\n\npython -m venv .venv\nsource .venv/bin/activate  # or .venv\\Scripts\\activate on Windows\npip install -r requirements.txt\npython main.py\n\n## Run with Docker\n\ndocker build -t ${projectName.toLowerCase().replace(/\s+/g, '-')}:latest .\ndocker run --rm ${projectName.toLowerCase().replace(/\s+/g, '-')}:latest\n`;
